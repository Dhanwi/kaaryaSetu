  ...
  return (
    <div className="space-y-4 bg-[#02101E] p-4 rounded-lg shadow-xl shadow-cyan-500/20">
      {education.map((obj, key) => (
        ...
        >
          <FaTimes
            className="absolute top-2 right-2 text-[#06B6D4] cursor-pointer"
            onClick={() => removeEducationField(key)}
          />
          <div className="flex flex-col space-y-1">
            <label className="text-sm text-[#22D3EE] font-medium flex items-center">
              <FaSchool className="mr-2" /> Institution Name #{key + 1}
            </label>
            <input
              type="text"
              placeholder={`Institution Name #${key + 1}`}
              className="placeholder-neutral-200 placeholder:text-sm border-y-2 text-[#22D3EE] hover:shadow-cyan-500 hover:shadow-sm rounded-lg bg-transparent border-cyan-500/50 p-2 w-full"
      ...
      <button
        ...
        className="mt-4 bg-[#22D3EE] hover:bg-[#0891B2] text-white py-2 px-4 rounded transition border border-cyan-500/50"
      >
