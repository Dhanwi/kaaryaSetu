...
const SkillInput = ({ signupDetails, setSignupDetails }) => {
  ...
  return (
    <div className="flex flex-col bg-[#02101E] p-4 rounded-lg shadow-xl shadow-cyan-500/20 w-full">
      <label className="text-sm md:text-base text-[#22D3EE] font-medium mb-1">
        Skills
      </label>
      <div className="flex flex-wrap items-center gap-2 p-2 border border-cyan-500/50 rounded bg-[#041124]">
        {signupDetails.skills &&
          signupDetails.skills.map((skill, index) => (
            <div
              key={index}
              className="flex items-center bg-[#020C1B] text-[#22D3EE] px-3 py-1 rounded-full cursor-pointer hover:bg-[#041124] text-sm md:text-base"
        ...
        <input
          ...
          className="flex-grow p-2 border-none focus:outline-none bg-transparent text-[#22D3EE] placeholder-neutral-200 placeholder:text-sm md:placeholder:text-base"
        />
      </div>
