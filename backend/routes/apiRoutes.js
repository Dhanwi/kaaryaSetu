...
router.post("/jobs", jwtAuth, (req, res) => {
  ...
  if (!data.jobDescription) {
    return res.status(400).send({ error: "Job description is required" });
  }
...
router.get("/jobs/:id", jwtAuth, (req, res) => {
  Job.findOne({ _id: req.params.id })
    .populate("userId", "hrMail hrContactNumber prompts")
    .then((job) => {
      if (job == null) {
        res.status(400).json({
...
router.put("/jobs/:id", jwtAuth, (req, res) => {
  ...
  Job.findOne({
    ...
    .then((job) => {
      ...
      if (data.hrMail !== undefined) {
        job.hrMail = data.hrMail;
      }
      if (data.hrContactNumber !== undefined) {
        job.hrContactNumber = data.hrContactNumber;
      }
      if (data.prompts) {
        job.prompts = data.prompts;
      }
      if (data.salary) {
      ...
      if (user.type === "recruiter") {
        Recruiter.findOne({ userId: user._id })
          .then((recruiter) => {
            ...
            res.json({ ...userDetails, ...recruiter.toObject() });
          })
        ...
        JobApplicant.findOne({ userId: user._id })
          .then((jobApplicant) => {
            ...
            res.json({ ...userDetails, ...jobApplicant.toObject() });
          })
          .catch((err) => res.status(400).json(err));
      }
    })
...
router.put("/jobs/:id/applications", jwtAuth, (req, res) => {
  const user = req.user;
  const jobId = req.params.id;
  ...
  if (user.type !== "applicant") {
    return res.status(401).json({
      message: "You don't have permissions to update the application status",
    });
  }
...
// Save a job
router.post("/jobs/:id/save", jwtAuth, (req, res) => {
  const user = req.user;
  const jobId = req.params.id;

  if (user.type !== "applicant") {
    return res.status(401).json({
      message: "You don't have permissions to save a job",
    });
  }
  ...
  Job.findOne({ _id: jobId })
    .then((job) => {
      ...
      const savedJob = new SavedJob({
        ...
        jobId: job._id,
        jobDetails: job,
        savedAt: new Date(), // Add savedAt field
      });
...
// Get saved jobs
router.get("/saved-jobs", jwtAuth, (req, res) => {
  const user = req.user;

  if (user.type !== "applicant") {
    return res.status(401).json({
      message: "You don't have permissions to view saved jobs",
    });
  }

  SavedJob.find({ userId: user._id })
    .then((savedJobs) => {
      res.json(savedJobs);
    })
    .catch((err) => {
...
// Update Password
router.post("/update-password", jwtAuth, (req, res) => {
